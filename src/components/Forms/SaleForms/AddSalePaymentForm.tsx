...
const AddSalePaymentForm: React.FC<UpdateFormInterface> = (props) => {
  ...
  paid:0
	...
	const createData = () => {
		apiClient()
         ...
         prescription_id:formData.sale_id
	...
	useEffect(() => {
        ...
        setFormData(
          {...formData,
            sale_id: props.value.id,
            amount:(props.value.calculatedSubTotal - props.value.calculatedDiscount),
            paid
	...
	const updateData = () => {
		apiClient()
			.post(`${BACKENDAPI}/v1.0/sale-payment`, { ...formData })
			.then((response: any) => {
				console.log(response);
